...
Plot_volcano <- function(Diff_res,Dir='./',save_Plot=TRUE){
  ...
  volcano_plot <- ggpubr::ggscatter(df,x = "log2FoldChange",y = "logP",color = "Group",alpha=0.8,
...
Plot_heatmap <- function(Diff_res,dds,Dir='./',save_Plot=TRUE,show_rownames=FALSE){
  ...
  if ( nrow(upExpression) <2 | nrow(downExpression) <2 ) {
    ...
    heat <- pheatmap::pheatmap(subexpression, show_rownames=show_rownames ,annotation_col=group, fontsize_row = 10, cluster_cols = F,
...
Plot_DE_GOTerms <- function(Diff_res,DIR='./',p_cutoff=0.05){
  ...
  pheatmap::pheatmap(test,cluster_cols=FALSE,cluster_rows=FALSE,annotation_row = anno,
           fontsize_row=22,
           fontsize_col=8,
  ...
  pheatmap::pheatmap(test2,cluster_cols=FALSE,cluster_rows=FALSE,annotation_row = anno,
           fontsize_row=22,
           fontsize_col=8,
           cellwidth = 40, cellheight = 30,
           show_colnames=F,
...
Barplot_ID <- function(dds,ID){
  ...
  ggpubr::ggboxplot(gene_dat, y = "NormCounts", x = "Condition",color = "Condition",
