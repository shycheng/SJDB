...
scBubbHeat <- function(shinycell_prefix, inp, inpH5, inpGrp, inpPlt = "Bubbleplot",
  ...
  if (inpRow) {
    ...
    ggRow <- ggplot() +
      ...
      scale_y_continuous(
        breaks = rep(0, data.table::uniqueN(ggData$grpBy)),
  ...
  if (inpCol) {
    ...
    ggCol <- ggplot() +
      ...
      scale_y_continuous(
        breaks = rep(0, data.table::uniqueN(ggData$geneName)),
  ...
  if (!save) {
    if (inpRow & inpCol) {
      ggOut <-
        gridExtra::grid.arrange(ggOut, ggLeg, ggCol, ggRow,
          widths = c(7, 1), heights = c(1, 7, 2),
          layout_matrix = rbind(c(3, NA), c(1, 4), c(2, NA))
        )
    } else if (inpRow) {
      ggOut <-
        gridExtra::grid.arrange(ggOut, ggLeg, ggRow,
          widths = c(7, 1), heights = c(7, 2),
          layout_matrix = rbind(c(1, 3), c(2, NA))
        )
    } else if (inpCol) {
      ggOut <-
        gridExtra::grid.arrange(ggOut, ggLeg, ggCol,
          heights = c(1, 7, 2),
          layout_matrix = rbind(c(3), c(1), c(2))
        )
    } else {
      ggOut <-
        gridExtra::grid.arrange(ggOut, ggLeg,
          heights = c(7, 2),
    ...
    if (inpRow & inpCol) {
      ggOut <-
        gridExtra::arrangeGrob(ggOut, ggLeg, ggCol, ggRow,
          widths = c(7, 1), heights = c(1, 7, 2),
          layout_matrix = rbind(c(3, NA), c(1, 4), c(2, NA))
        )
    } else if (inpRow) {
      ggOut <-
        gridExtra::arrangeGrob(ggOut, ggLeg, ggRow,
          widths = c(7, 1), heights = c(7, 2),
          layout_matrix = rbind(c(1, 3), c(2, NA))
        )
    } else if (inpCol) {
      ggOut <-
        gridExtra::arrangeGrob(ggOut, ggLeg, ggCol,
          heights = c(1, 7, 2),
          layout_matrix = rbind(c(3), c(1), c(2))
        )
    } else {
      ggOut <-
        gridExtra::arrangeGrob(ggOut, ggLeg,
          heights = c(7, 2),
          layout_matrix = rbind(c(1), c(2))
        )
    }
